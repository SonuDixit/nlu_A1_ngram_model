...
allSents=gutenberg.sents()
totalNumSent = len(allSents)
allSentIndex=range(0,len(allSents))
trainSentIndex=random.sample(range(0,len(allSents)),int(0.80 * totalNumSent))
trainSent=[]
...
allSents_guten=brown.sents()
totalNumSent_guten = len(allSents_guten)
import string
punc = string.punctuation


trainSent = trainSent+allSents_guten
...
print('perplexity of gutenTest is(using bigram):',math.exp(perPlex/totWord))
...
print('perplexity of gutenTest is(using trigram):',math.exp(perPlexTri/totWordTri))
